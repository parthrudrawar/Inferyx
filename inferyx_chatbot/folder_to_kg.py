...
Use PascalCase for node types and UPPERCASE for relationship types.
...
# 🧼 Helper functions for Neo4j formatting
def sanitize_label(name: str) -> str:
    return ''.join(e for e in name.strip().title() if e.isalnum())
...
async def run_kg_extraction(csv_dir="/app/framework/upload/user_files"):
    ...
    for node in all_nodes:
        try:
            ...
            label = sanitize_label(node.node_type)
            props = ", ".join([f"{sanitize_property(p)}: '{p}'" for p in node.properties])
            cypher = f"""
            MERGE (n:{label} {{internal_id: {node.node_type_id}}})
            SET n += {{
                {props},
                name: '{label}'
            }}
    ...
    for rel in all_edges:
        try:
            source_label = sanitize_label(rel.source)
            target_label = sanitize_label(rel.target)
            edge_label = sanitize_label(rel.edge_type).upper()
            cypher = f"""
...
if __name__ == "__main__":
    ...
    # 📦 Print KG JSON
    import json
    class KGJson(BaseModel):
        nodes: List[GraphNode]
        relationships: List[GraphRelationship]

    kg_json = KGJson(nodes=nodes, relationships=edges)
    print("\n=== 📦 KG JSON ===")
    print(json.dumps(kg_json.dict(), indent=2))
